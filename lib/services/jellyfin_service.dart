...
class JellyfinService extends ChangeNotifier {
  ...
  Future<List<MediaItemModel>> getTracksByGenre(String genre) async {
    final response = await http.get(
      Uri.parse('$_serverUrl/Users/$_userId/Items?IncludeItemTypes=Audio&Recursive=true&Limit=20&SortBy=Random&Genres=${Uri.encodeQueryComponent(genre)}&ImageTypeLimit=1&EnableImageTypes=Primary'),
      headers: _headers,
    );
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      return (data['Items'] as List).map((i) => MediaItemModel.fromJson(i, _serverUrl!)).toList();
    }
    return [];
  }

  Future<List<MediaItemModel>> getRandomTracks({int limit = 20}) async {
    final response = await http.get(
      Uri.parse('$_serverUrl/Users/$_userId/Items?IncludeItemTypes=Audio&Recursive=true&Limit=$limit&SortBy=Random&ImageTypeLimit=1&EnableImageTypes=Primary'),
      headers: _headers,
    );
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
  ...
  Future<List<String>> getGenres() async {
    final response = await http.get(
      Uri.parse('$_serverUrl/MusicGenres?UserId=$_userId&Recursive=true&Limit=10&SortBy=SortName'),
      headers: _headers,
    );
    if (response.statusCode == 200) {
      final data = jsonDecode(response.body);
      return (data['Items'] as List).map<String>((i) => i['Name'] as String).toList();
    }
    return [];
  }
}
